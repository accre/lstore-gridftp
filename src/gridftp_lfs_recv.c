...
//rc = 1;
    ...
    if (rc > 1) {
        ...
        it = iter_search_interval_skiplist(written_intervals, (skiplist_key_t *)NULL,
                                           (skiplist_key_t *)NULL);
        i = 0;
    ...
    log_printf(1, "rc=%d last_byte=" XOT
               " done_status=%d GLOBUS_SUCCESS=%d adler32=%s\n", rc, last_byte,
               lfs_handle->done_status, GLOBUS_SUCCESS, lfs_handle->adler32_human);
...
globus_result_t prepare_handle(lfs_handle_t *lfs_handle)
{
    ...
    if (!lfs_handle->pathname_munged) {
        MemoryError(lfs_handle, "Unable to make a copy of the path name.", rc);
        return rc;
    }
