...
//       push_link(lfs_handle->backend_stack.stack, ele);  // ** Add the sentinel
    ...
    if ((nbytes != (unsigned) lfs_handle->buffer_size)
        ...
        log_printf(1, "Triggering shutdown.  rc=%d GLOBUS_SUCCESS=%d\n", rc,
                   GLOBUS_SUCCESS);
        globus_gridftp_server_finished_transfer(lfs_handle->op,
                                                rc);  // ** This can trigger a GridFTP segfault but it looks to be a gridftp problem not LIO.
    ...
    while ((finished == 0) && (total_left > 0)) {
        ...
        if (nbytes != nread) {
            globus_gfs_log_message(GLOBUS_GFS_LOG_ERR,
                                   "Failed to read! offset=" XOT " nbytes=" XOT " nread=" XOT "\n", offset, nbytes,
                                   nread);
            SystemError(lfs_handle, "reading from posix", rc);
            buf->nbytes = 0;
            buf->offset = 0;
            buf->eof =
                GLOBUS_FAILURE;  // ** This can trigger an error in GridFTP but it's not a bug in the plugin!
            nbytes = offset = 0;
            finished = 1;
            oops = EREMOTEIO;
